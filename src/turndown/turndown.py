...
class TurndownService:
    ...
    def turndown(self, input_obj):
        ...
        if not _can_convert(input_obj):
            raise TypeError(f"{input_obj} ist kein String oder ein valider Node.")
    ...
    def _post_process(self, output):
        ...
        self.rules.forEach(
            lambda r, i: (
                nonlocal_output.__setitem__(0, _join(nonlocal_output[0], do_append(r)))
                if do_append(r)
                else None
            )
        )
        ...
        nonlocal_output = [output]
        self.rules.forEach(lambda r, i: self._append_references(r, nonlocal_output))
        return nonlocal_output[0]
